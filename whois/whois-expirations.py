...
def parse_date(s):
    ...
    elif re.match(rb'^\d\d/\d\d/\d\d\d\d \d\d:\d\d:\d\d', s):
        # 20/05/2014 00:00:00 EEST 
        # 18/08/2016 23:59:59
        return datetime.strptime(s[:19].decode('utf-8'), '%d/%m/%Y %H:%M:%S')
    elif re.match(rb'^\d\d/\d\d/\d\d\d\d \d\d:\d\d:\d\d.\d\d\d\d\d\d [A-Z]+$', s):
...
def main(argv):
    ...
    while line != b'':
        if state == WarcReadStates.EXPECTING_HEADER:
            ...
            m = re.match(rb'^(?i)Content-Length: (\d+)\r\n$', line)
            if m != None:
                content_length = int(m.group(1))
            m = re.match(rb'^(?i)WARC-Target-URI: (.*)\r\n$', line)
            if m != None:
                url = m.group(1)
            ...
            if url != None and url.startswith(b'whois://') and not re.match(rb'^.*\d+\.\d+\.\d+\.\d+$', url):
                ...
                #
                m = re.search(rb'(?im)(?:exp|valid|renewal)[^:\n]*?(?: on \s*:|:| on )\s*([^\r\n]+)', line)

                if m != None:
                    d = parse_date(m.group(1))
                    if d != None:
                        url_parts = re.split(rb'[/+]', url)
                        domain = url_parts[-1]
